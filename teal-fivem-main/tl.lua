---@diagnostic disable: param-type-mismatch, undefined-field, cast-local-type, deprecated, missing-return, need-check-nil, undefined-global
...
local function search_for(module_name, suffix, tried)
   local slash_name = module_name:gsub("%.", "/")
   local tl_filename = slash_name:gsub("%.lua$", suffix)
   local fd = LoadResourceFile(GetCurrentResourceName(), tl_filename..'.tl')
...
function tl.search_module(module_name, search_dtl)
   ...
   local tried = {}
   if search_dtl then
      found, fd, tried = search_for(module_name, ".d.tl", tried)
      if found then
         return found, fd
      end
   end
   found, fd, tried = search_for(module_name, ".tl", tried)
   if found then
      return found, fd
   end
   found, fd, tried = search_for(module_name, ".lua", tried)
   if found then
      return found, fd
   end
   return nil, nil, tried
...
function tl.loader(module)
   tl_package_loader(module)
end
